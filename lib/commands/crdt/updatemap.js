...
var MapOp = require('../../protobuf/riakprotobuf').getProtoFor('MapOp');
var MapField = require('../../protobuf/riakprotobuf').getProtoFor('MapField');
var MapUpdate = require('../../protobuf/riakprotobuf').getProtoFor('MapUpdate');
var CounterOp = require('../../protobuf/riakprotobuf').getProtoFor('CounterOp');
var SetOp = require('../../protobuf/riakprotobuf').getProtoFor('SetOp');
var FlagOp = require('../../protobuf/riakprotobuf').getProtoFor('FlagOp');
 ...
 * @param {MapOperation} op the set of modifications to make to this map.
 ...
 * @param {Boolean} [options.returnBody=false] return the map.
 ...
 * @param {Number} callback.response the map from Riak if returnbody is set.
 * @extends CommandBase
 */
...
UpdateMap.prototype.onRiakError = function(rpbErrorResp) {
    this.onError(rpbErrorResp.getErrmsg().toString('utf8'));
};
    
    
UpdateMap.prototype.onError = function(msg) {
...
UpdateMap.prototype._populate = function(mapOp, pbMapOp) {
    ...
    for (i = 0; i < mapOp.sets.removes.length; i++) {
        ...
        for (j = 0; j < mapOp.sets.removes[i].remove.length; j++) {
            v = mapOp.sets.removes[i].remove[j];
            if (typeof v === 'string') {
                v = new Buffer(v);
            }
 ...
 * 
 *      var UpdateMap = require('./lib/commands/datatype/updateMap');
 *      var update = new UpdateMap.Builder()
...
Builder.prototype = {
     ...
     * @param {String} callback.err An error message
     * @param {Object} callback.response - the response from Riak if returnbody is set to true.
     * @chainable
     */
...
MapOperation.prototype = {
     ...
     * @param {Buffer} value the value for the register.
     * @chainable}
     */
