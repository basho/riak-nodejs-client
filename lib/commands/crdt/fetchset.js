 ...
 * @param {Boolean} returnRaw return buffers instead of strings.
...
FetchSet.prototype.onSuccess = function(dtFetchResp) {
    ...
    {
        var bbArray = dtFetchResp.getValue().getSetValue();
        if (this.options.returnRaw) {
            var arrayToReturn = bbArray;
        } else {
            var arrayToReturn = new Array( bbArray.length);
            for (var i = 0; i < bbArray.length; i++) {
                arrayToReturn[i] = bbArray[i].toString('utf8');
            }
        }
...
bb('timeout');
/**
 * Return Buffer objects instead of strings.
 * By default the returned set with contain utf8 strings. The 
 * underlying data is stored as bytes and by setting this to true 
 * they will be returned unconverted as Buffer objects.
 * @method withReturnRaw
 * @param {Boolean} returnRaw true to return Buffers instead of strings.
 * @chainable
 */
bb('returnRaw');
