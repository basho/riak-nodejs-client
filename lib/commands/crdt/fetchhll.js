...
/**
 * Provides the FetchHll class, its builder, and its response.
 * @module CRDT
 */

/**
 *  Command used to fetch a hyperloglog value from Riak.
 *
 ...
 *                      .build();
 *
 * See {{#crossLink "FetchHll.Builder"}}FetchHll.Builder{{/crossLink}}
 *
 * @class FetchHll
 * @constructor
 ...
 * @param {String} options.key The key for the hyperloglog you want to fetch.
 ...
 * @param {Boolean} callback.response.isNotFound True if there was no hyperloglog in Riak.
 * @param {Object} callback.data additional error data. Will be null if no error.
 * @extends CommandBase
 */
function FetchHll(options, callback) {
    CommandBase.call(this, 'DtFetchReq', 'DtFetchResp', callback);
    this.validateOptions(options, schema);
}

inherits(FetchHll, CommandBase);

FetchHll.prototype.constructPbRequest = function() {
...
FetchHll.prototype.onSuccess = function(dtFetchResp) {
    var cardinality = null;
    var isNotFound = false;
    // on a "not found" the value is null

    if (dtFetchResp.value) {
      cardinality = dtFetchResp.value.hll_value.toNumber();
    } else {
      isNotFound = true;
    }

    this._callback(null, { cardinality: cardinality, isNotFound: isNotFound });
...
/**
 * A builder for constructing FetchHll instances.
 *
 * Rather than having to manually construct the __options__ and instantiating
 * a FetchHll directly, this builder may be used.
 *
 *     var fetch = new FetchHll.Builder()
 *                      .withBucketType('myBucketType')
 *                      .withBucket('myBucket')
...
Builder.prototype = {
    ...
    */
    ...
    build : function() {
        ...
        return new FetchHll(this, cb);
    }
};

module.exports = FetchHll;
