...
/**
 * Provides the GetBucketProps class, its builder, and its response.
 * @module GetBucketProps
 */
...
FetchBucketProps.prototype.onSuccess = function(rpbGetBucketResp) {
    ...
    var backend;
    if (backend = rpbBucketProps.getBackend()) {
        bucketProps.backend = backend.toString('utf8');
    }
    
    var searchIndex;
    if (searchIndex = rpbBucketProps.getSearchIndex()) {
        bucketProps.searchIndex = searchIndex.toString('utf8');
    }
    
    var dataType;
    if (dataType = rpbBucketProps.getDatatype()) {
 ...
 * a GetBucketProps directly, this builder may be used.
 *
 * 
...
Builder.prototype = {
    ...
    /**
     * Construct a GetBucketProps instance.
