                ...
                for (var j = 0; j < rcells.length; j++) {
                    ...
                    switch (coltype) {
                        case TsColumnType.BINARY:
                            ...
                            if (binaryValue) {
                                ...
                                if (this.options.convertToStrings) {
                                    rsprow.push(binaryValue.toString('utf8'));
                                } else {
                        ...
                        case TsColumnType.TIMESTAMP:
                            value = rcell.getTimestampValue();
                            if (!value) {
                                value = rcell.getIntegerValue();
                            }
                            rsprow.push(value);
                            break;
                        ...
                        case TsColumnType.SET:
                            var setValue = rcell.getSetValue();
                            if (setValue) {
                                if (Array.isArray(setValue)) {
                                    var set = [];
                                    if (setValue.length > 0) {
                                        var setValueFromJSON = makeSetValueDeserializer(set);
                                        setValue.forEach(setValueFromJSON);
                                    }
                                    rsprow.push(set);
                                } else {
                                    rsprow.push(JSON.parse(setValue.toString('utf8')));
                                }
                            } else {
                                rsprow.push(null);
                            }
                        ...
                        case TsColumnType.MAP:
                            var mapValue = rcell.getMapValue();
                            if (mapValue) {
                                rsprow.push(JSON.parse(mapValue.toString('utf8')));
                            } else {
                                rsprow.push(null);
                            }
                            break;
