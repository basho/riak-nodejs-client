...
RiakCluster.prototype.start = function() {
    ...
    if (this._state === State.RUNNING) {
        ...
        for (var i = 0; i < this.nodes.length; i++) {
            this.nodes[i].on(EVT_SC, this._onNodeStateChange.bind(this));
            this.nodes[i].on(EVT_RC, this._onRetryCommand.bind(this));
            this.nodes[i].start();
        }
...
var defaultNodeManager = new DefaultNodeManager();
