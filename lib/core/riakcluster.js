...
RiakCluster.prototype.execute = function(riakCommand, previous) {
    ...
    logger.debug('[RiakCluster] command %s executing %s',
        riakCommand.PbRequestName, executing);
    ...
    if (!executing) {
        if (this.queueCommands) {
            if (this.queueMaxDepth && (this._commandQueue.length >= this.queueMaxDepth)) {
                ...
                logger.debug('[RiakCluster] queuing command %s', riakCommand.PbRequestName);
...
RiakCluster.prototype._onRetryCommand = function(command, lastNode) {
    logger.debug('[RiakCluster] retrying command %s', command.PbRequestName);
