...
function makeStartNodeFunc(cluster, node) {
    var f = function (async_cb) {
        cluster._startNode(node, function (err, rslt) {
            if (err) {
                logger.error('[RiakCluster] _startNode - error starting node (%s:%d):', 
 ...
 * @method stop
 */
// TODO add callback for when fully stopped
...
RiakCluster.prototype.addNode = function(node) {
    ...
    this._startNode(node, function (err, rslt) {
        if (err) {
            logger.error('[RiakCluster] addNode - error starting node (%s:%d):', 
                node.remoteAddress, node.remotePort, err);
        }
