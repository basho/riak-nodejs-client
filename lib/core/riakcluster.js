 ...
 * @param {Number} [options.queueTimeout=500] Time in milliseconds to re-try executing queued commands. Default is 500.
 * 
 */
function RiakCluster(options) {
    ...
    Joi.validate(options, schema, function(err, options) {
        ...
        self.queueTimeout = options.queueTimeout;
    });
        ...
        if (this.queueCommands) {
            if (this.queueMaxDepth && (this._commandQueue.length >= this.queueMaxDepth)) {
                ...
                if (this.state === State.RUNNING) {
                    ...
                    setTimeout(this._submitFromQueue.bind(this), this.queueTimeout);
...
var schema = Joi.object().keys({
    ...
    queueTimeout: Joi.number().default(500)
});
...
Builder.prototype = {
    ...
    withQueueTimeout : function(timeout) {
