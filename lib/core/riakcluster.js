...
RiakCluster.prototype.execute = function(riakCommand, previous) {
    ...
    logger.debug('[RiakCluster] execute command: %s remaining tries: %d.',
        riakCommand.name, riakCommand.remainingTries);
...
RiakCluster.prototype._onRetryCommand = function(command, lastNode) {
    logger.debug('[RiakCluster] scheduling re-try for: ', command.name);
    // TODO: backoff per-retrycount?
    setTimeout(this.execute.bind(this, command, lastNode), 125);
};
