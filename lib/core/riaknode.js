...
RiakNode.prototype.execute = function(command) {
    ...
    if (!conn) {
        ...
        logger.info("Executing command on RiakNode (%s:%d)", this.remoteAddress, this.remotePort);
...
RiakNode.prototype._responseReceived = function(conn, command, code, decoded) {
    ...
    if (code === RiakProtobuf.getCodeFor('RpbErrorResp')) {
        ...
        logger.info('RiakNode (%s:%d): recevied RpbErrorResp for command; %s', this.remoteAddress, this.remotePort, decoded.getErrmsg().toString('utf8'));
        if (command.remainingTries) {
