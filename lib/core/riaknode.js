...
RiakNode.prototype._shutdown = function(callback) {
    ...
    if (this._shutdownCount > 10 ||
        this._currentNumConnections <= 0 || 
        (this.executeCount === 0 && this._available.length === 0)) {
        this.state = State.SHUTDOWN;
        logger.debug("[RiakNode] (%s:%d:%d:%d:%d) shut down.",
            this.remoteAddress, this.remotePort, this.executeCount,
            this._currentNumConnections, this._shutdownCount);
        ...
        if (this._currentNumConnections > 0) {
            logger.warn('[RiakNode] (%s:%d) connection count (%d) NOT ZERO at shutdown',
                this.remoteAddress, this.remotePort, this._currentNumConnections);
        }
        this.emit(EVT_SC, this, this.state);
        ...
        logger.debug("[RiakNode] (%s:%d) connections still in use (%d:%d:%d)",
            ...
            this._currentNumConnections, this._available.length,
            this.executeCount);
        setTimeout(this._shutdown.bind(this, callback), 100);
    }
};
...
RiakNode.prototype._returnConnectionToPool = function (conn) {
    if (this.state < State.SHUTTING_DOWN) {
        ...
        logger.debug("[RiakNode] node (%s:%d) _available: %d, _currentNum: %d",
            this.remoteAddress, this.remotePort,
            this._available.length, this._currentNumConnections);
    } else {
