...
function RiakNode(options) {
    ...
    this._getConnectionFromPool = function (cmd_name) {
        ...
        do {
            ...
            if (conn.closed) {
                logger.warn('[RiakNode] (%s:%d) execute of %s attempted on closed connection',
                    this.remoteAddress, this.remotePort, cmd_name);
                conn = null;
            }
        } while (!conn);
        return conn;
    ...
    this._returnConnectionToPool = function (conn) {
        if (this.state < State.SHUTTING_DOWN) {
            ...
            logger.debug("[RiakNode] node (%s:%d) _returnConnectionToPool a: %d, cnc: %d",
                this.remoteAddress, this.remotePort,
                available.length, currentNumConnections);
        } else {
    ...
    this._expireIdleConnections = function () {
        ...
        while (available.next() && currentNumConnections > this.minConnections) {
            ...
            if (curr.closed || (now - curr.lastUsed >= this.idleTimeout)) {
                var conn = available.removeCurrent();
                currentNumConnections--;
                if (!conn.closed) {
                    conn.close();
...
RiakNode.prototype._createNewConnection = function (postConnectFunc, postFailFunc, healthCheck) {
    ...
    conn.on('connected', function (conn) {
        logger.debug("[RiakNode] (%s:%d) conn.on-connected",
            this.remoteAddress, this.remotePort);
    ...
    conn.on('connectFailed', function (conn, err){
       logger.debug("[RiakNode] (%s:%d) conn.on-connectFailed",
            this.remoteAddress, this.remotePort);
...
RiakNode.prototype._shutdown = function(callback) {
    ...
    if (this._shutdownCount > 10 || this._isPoolEmpty()) {
        ...
        if (this.executeCount > 0) {
            logger.warn('[RiakNode] (%s:%d) execution count (%d) NOT ZERO at shutdown',
                this.remoteAddress, this.remotePort, this.executeCount);
        }
        if (pi.cnc > 0) {
            logger.warn('[RiakNode] (%s:%d) connection count (%d) NOT ZERO at shutdown',
                this.remoteAddress, this.remotePort, pi.cnc);
        }
        this.emit(EVT_SC, this, this.state);
        ...
        logger.debug("[RiakNode] (%s:%d) connections still in use (%d:%d:%d)",
            this.remoteAddress, this.remotePort,
            pi.cnc, pi.count,
            this.executeCount);
...
RiakNode.prototype.execute = function (command) {
    ...
    logger.debug("[RiakNode] executing command '%s' on node (%s:%d)",
        command.name, this.remoteAddress, this.remotePort);
    ...
    if (this.state === State.RUNNING) {
        ...
        if (!conn) {
            if (this._createNewConnectionAllowed()) {
                ...
                this._createNewConnection(function (newConn) {
                    logger.debug("[RiakNode] executing command '%s' on node (%s:%d:%d) (new connection)",
                        command.name, self.remoteAddress, self.remotePort, self.executeCount);
            ...
            logger.debug("[RiakNode] executing command '%s' on node (%s:%d:%d) (existing connection)",
                command.name, this.remoteAddress, this.remotePort, this.executeCount);
...
RiakNode.prototype._responseReceived = function (conn, command, code, decoded) {
    logger.debug("[RiakNode] node (%s:%d:%d) command '%s' received code: %d",
        this.remoteAddress, this.remotePort, this.executeCount,
        command.name, code);
    ...
    if (code === RiakProtobuf.getCodeFor('RpbErrorResp')) {
        ...
        if (logger.debug) {
            ...
            logger.debug("[RiakNode] node (%s:%d) command '%s' received RpbErrorResp (%d) %s",
                this.remoteAddress, this.remotePort, command.name, errcode, errmsg);
        }
        ...
        if ((hasDone && decoded.done) || !hasDone) {
            ...
            logger.debug('[RiakNode] node (%s:%d:%d) command %s complete',
                this.remoteAddress, this.remotePort, this.executeCount, command.name);
        }
...
RiakNode.prototype._connectionClosed = function (conn) {
    ...
    logger.debug("[RiakNode] node (%s:%d) connection closed; in-flight: %s", this.remoteAddress, this.remotePort, conn.inFlight);
    if (conn.inFlight) {
        this.executeCount--;
        var cmd = conn.command;
        this._maybeRetryCommand(cmd, function () {
            cmd.onError("Connection closed while executing command");
        });
    }
    if (this.state !== State.SHUTTING_DOWN) {
...
RiakNode.prototype._doHealthCheck = function () {
    ...
    setTimeout(this._healthCheck.bind(this), 75);
};
...
RiakNode.prototype._healthCheck = function () {
    ...
    this._createNewConnection(function (newConn) {
        ...
        setTimeout(self._healthCheck.bind(self), 75); // TODO should HC interval increase by some percentage each iteration up to a max?
    }, this.healthCheck);
};
...
RiakNode.prototype._maybeRetryCommand = function (command, errfunc) {
    ...
    logger.debug("[RiakNode] node (%s:%d) command %s remaining tries %d -> %d",
        this.remoteAddress, this.remotePort, command.name, tries, command.remainingTries);
