...
function RiakConnection(options) {
    ...
    if (this.tls_options) {
        this.tls_state = TlsStateEnum.SEND_STARTTLS;
    } else {
        this.tls_state = TlsStateEnum.NO_TLS;
    }
...
RiakConnection.prototype._connected = function() {
    ...
    if (this.tls_state === TlsStateEnum.SEND_STARTTLS) {
        // TODO: idea - this._connection.on('data', this._receiveStartTls.bind(this));
        this.tls_state = TlsStateEnum.AWAIT_STARTTLS;
        logger.debug("Sending RpbStartTls");
...
RiakConnection.prototype._receiveData = function(data) {
    ...
    if (this.tls_state === TlsStateEnum.NO_TLS || this.tls_state == TlsStateEnum.COMPLETE) {
        ...
        switch (this.tls_state) {
            case TlsStateEnum.AWAIT_STARTTLS:
                // https://nodejs.org/api/tls.html#tls_new_tls_tlssocket_socket_options
                // https://github.com/iojs/io.js/issues/878
                var tls_secure_context = tls.createSecureContext(this.tls_options);
                var tls_socket_options = {
                    isServer: false, // NB: required
                    secureContext: tls_secure_context,
                };
                // TODO https://github.com/joyent/node/issues/9242
                this._connection = new tls.TLSSocket(this._connection, tls_socket_options);
                this.tls_state = TlsStateEnum.AWAIT_AUTH;
                logger.debug("Sending AuthReq");
                var auth_options = {
                    user: this.tls_options.user,
                    password: this.tls_options.password,
                };
                var command = new AuthReq(auth_options);
                this.inFlight = false;
                this.execute(command);
                break;
            case TlsStateEnum.AWAIT_AUTH:
                this.tls_state = TlsStateEnum.COMPLETE;
                break;
            default:
                // TODO expand
                throw 'Error in TLS session startup.';
        }
    }
