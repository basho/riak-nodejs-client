...
RiakConnection.prototype._connected = function() {
    ...
    if (this.auth) {
        ...
        this._connection.on('data', this._receiveStartTls.bind(this));
...
RiakConnection.prototype._receiveStartTls = function(data) {
    ...
    // On data, move to next sequence in TLS negotiation
    this._connection.removeListener('data', this._receiveStartTls);
    this._connection.on('data', this._receiveAuthResp.bind(this));
...
RiakConnection.prototype._receiveAuthResp = function(data) {
    ...
    logger.debug('_receiveAuthResp');

    this._ensureExpectedResponse(data, 'RpbAuthResp');

    logger.debug('TLS negotiation and auth COMPLETE');

    // On data, use the _receiveData function
    this._connection.removeListener('data', this._receiveAuthResp);
    this._connection.on('data', this._receiveData.bind(this));
    this._connection.setTimeout(0);

    this.inFlight = false;

    // Clean up functions that should be unavailable from here on out
    delete RiakConnection.prototype._receiveStartTls;
    delete RiakConnection.prototype._receiveAuthResp;
    delete RiakConnection.prototype._ensureExpectedResponse;
...
RiakConnection.prototype._receiveData = function(data) {
    ...
    for (var i = 0; i < protobufArray.length; i++) {
        logger.debug("emitting responseReceived event");
