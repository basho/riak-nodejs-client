...
function RiakConnection(options) {
    ...
    // NB: auth options are validated in RiakNode
    if (options.auth) {
...
function _executeAuthRequest(self) {
    var auth_options = {
        user: self.auth.user,
...
RiakConnection.prototype._connected = function() {
    ...
    if (this.auth) {
         ...
         */
        if (this.auth.start_tls) {
            logger.debug('%s setting up legacy TLS (%s)', this.name, this.auth.tls);
            ...
            if (!this.auth.start_tls) {
                _setupTlsSocket(this);
            }
            logger.debug('%s authenticating, TLS not required or already started', this.name);
            _executeAuthRequest(this);
        }
...
RiakConnection.prototype._startTls = function(conn, command, code, decoded) {
};
